...
public abstract class ApiObserver<T> implements Observer<ResultInfo<T>> {
    ...
    public void onNext(ResultInfo<T> responseBody) {
        ...
        if (null == responseBody) { //服务器错误
            onApiError(new NetException.ResponseException(-1, "返回数据为空"));
        } else { //判断返回码的状态
    ...
    public void onError(Throwable e) {
        ...
        onApiError(NetException.handleException(e));
    }
    ...
    public abstract void onApiError(NetException.ResponseException exception);
}
